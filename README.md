...
- フォルダ階層のブラウズ、**リスト / アイコン（グリッド）表示**切替（切替ボタンの**長押し（メニュー）から、表示中の一覧をクリップボードにコピー**）
...
- **先読みバッファ**でシーク・早送りを安定化
- **シークバーのドラッグ**は指を離した位置へシーク（ドラッグ中は再生位置を変えず移動）。ドラッグ位置のフレームをサムの上に**サムネでプレビュー表示**（画像生成器を 1 本に使い回して接続枯渇を防止。ストリーミングは生成がやや遅延）
