...
- トップに **「ダウンロード済み」**（オフライン視聴一覧）と **「お気に入り」**（フォルダ長押しで登録）項目

### 再生（iOS カスタムプレイヤー）
- タップでコントロール表示、その間だけタイトル表示。一定時間で自動非表示
- **先読みバッファ**でシーク・早送りを安定化、シーク中は映像がライブ追従
- **横スワイプでシーク**（コントロール表示中も可。画面の上半分/下半分で秒数単位、**設定で変更可**）。シーク中はシークバー表示で映像がライブ追従
...
- 長押しメニューから **このシーンをサムネイルにする** / **タグを編集** / **このシーンを調べる**（後述）
